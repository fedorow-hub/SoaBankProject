    ...
    public class ClientInfoViewModel : ViewModel
    {
        ...
        private async void OnSaveCommandExecute(object p)
        {
            if (CurrentClient is null) // новый клиент
            {
                ...
                if (ClientAction.CreateClient(newClient))
                {
                    MessageBox.Show($"{_worker} создал клиента: {_firstname} {_lastname}");
                }
                else
                {
                    MessageBox.Show($"{_worker} не смог создать клиента: {_firstname} {_lastname}");
                }
            }
            else
            {
                ...
                if (ClientAction.UpdateClient(updatedClient))
                {
                    MessageBox.Show($"{_worker} обновил данные клиента: {_firstname} {_lastname}");
                }
                else
                {
                    MessageBox.Show($"{_worker} не смог обновить данные клиента: {_firstname} {_lastname}");
                }
            }
